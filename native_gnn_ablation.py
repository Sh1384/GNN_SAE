...
Performs ablations directly in native 64-dimensional GNN activation space,
...
def load_sae_and_activations(variant: str, config: Dict, graph_id: int, use_mixed_motifs: bool = False) -> Tuple:
    ...
    if use_mixed_motifs:
        ...
        activation_file = Path(f"outputs/activations/layer2/mixed/graph_{graph_id}.pt")
        ...
        activation_file = Path(f"outputs/activations/layer2/test/graph_{graph_id}.pt")
        if not activation_file.exists():
    ...
    if activations.ndim != 2 or activations.shape[1] != 64:
        raise ValueError(f"Invalid activation shape {activations.shape}, expected (N, 64)")
...
def patch_salient_nodes(activations: torch.Tensor, sae_model, feature_idx: int,
    ...
    Args:
        activations: Native GNN activations (num_nodes, 64)
...
def compute_native_rpb(test_graph_ids: List[int], motif: str) -> pd.DataFrame:
    ...
    activations_array = np.array(all_activations)  # (n_nodes, 64)
...
def compute_gnn_loss(activations: torch.Tensor, gnn_model, graph_id: int) -> float:
    ...
    2. Run GNN layer3 (conv3) using the given layer2 activations
    3. Compute MSE between GNN predictions and ground truth values on masked nodes

    Args:
        activations: Layer2 activations (num_nodes, 64) - either original or ablated
    ...
    try:
        ...
        # Run GNN layer3 (conv3) on the given activations
        with torch.no_grad():
            h3 = gnn_model.conv3(activations, edge_index, edge_weight=edge_weight)
            pred = h3.squeeze(-1)
        ...
        if Path(gnn_checkpoint_path).exists():
            try:
                ...
                if 'conv1.bias' not in state_dict or 'conv2.bias' not in state_dict:
                    ...
                    gnn_model = GCNModel(input_dim=2, hidden_dim1=hidden_dim1, hidden_dim2=hidden_dim2, output_dim=1, dropout=0.5)
...
Strategy 2 (NEW - Native r_pb Guided):
  ...
  → This bypasses SAE encoder entirely and directly measures native neuron importance
    ...
    if args.native_rpb:
        ...
        try:
            act_file = Path(f"outputs/activations/layer2/test/graph_{graph_ids[0]}.pt")
