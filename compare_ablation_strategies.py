...
def load_sae_motif_results(motif: str, latent_dim: int, k: int, variant: str = None) -> Optional[pd.DataFrame]:
    ...
    Expected file: ablations/results/{motif}_l{latent_dim}_k{k}_results.csv (no variant)
                   or ablations/results/{motif}_{variant}_l{latent_dim}_k{k}_results.csv (with variant)
    (This is the output from run_interpretability_experiments.py when ablating motif feature groups)
    """
    motif_display = motif.replace('in_', '').replace('_', ' ').title()
    motif_name = motif_display.lower().replace(' ', '_')

    # Try to load with variant first (new format)
    if variant:
    ...
    # Fall back to loading without variant (old format or when variant not specified)
