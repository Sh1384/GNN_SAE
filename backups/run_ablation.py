...
def load_sae_model(variant=None, checkpoint_path=None, latent_dim=None, **kwargs):
    ...
    try:
        if variant == 'topk':
            ...
            model = JumpReLUSAE(input_dim=64, latent_dim=latent_dim,
                               threshold_init=kwargs.get('threshold_init', 0.01),
                               bandwidth=kwargs.get('bandwidth', 0.01))
        elif variant == 'switch':
...
def run_ablation_experiment(variant, latent_dim, ablate_indices, experiment_name, motif_type_filter=None, use_mixed_motifs=False, variant_kwargs=None):
    ...
    try:
        if use_mixed_motifs:
            ...
            # These should be in outputs/activations/layer3_new/mixed/
            mixed_dir = Path('outputs/activations/layer3_new/mixed')
    ...
    for graph_id in tqdm(graph_ids):
        try:
            ...
            if use_mixed_motifs:
                ...
                act_file = Path(f"outputs/activations/layer3_new/mixed/graph_{graph_id}.pt")
            else:
                # Single motifs use layer2/test
                act_file = Path(f"outputs/activations/layer3_new/test/graph_{graph_id}.pt")
