...
def load_sae_and_activations(variant: str, config: Dict, graph_id: int, use_mixed_motifs: bool = False) -> Tuple:
    ...
    if use_mixed_motifs:
        ...
        activation_file = Path(f"outputs/activations/layer3_new/mixed/graph_{graph_id}.pt")
        ...
        activation_file = Path(f"outputs/activations/layer3_new/test/graph_{graph_id}.pt")
        if not activation_file.exists():
...
def compute_gnn_loss(activations: torch.Tensor, gnn_model, graph_id: int) -> float:
    ...
    2. Run GNN layer4 (conv4) using the given layer3 activations
    3. Compute MSE between GNN predictions and ground truth values on masked nodes

    Args:
        activations: Layer3 activations (num_nodes, 64) - output of conv3, either original or ablated
    ...
    try:
        ...
        # Run GNN layer4 (conv4) on the given layer3 activations to get final predictions
        with torch.no_grad():
...
Strategy 2 (NEW - Native r_pb Guided):
  ...
  → This bypasses SAE encoder entirely and directly measures native neuron importance
    ...
    if args.native_rpb:
        ...
        try:
            act_file = Path(f"outputs/activations/layer3_new/test/graph_{graph_ids[0]}.pt")
