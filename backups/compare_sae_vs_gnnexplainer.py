...
def load_top_features_from_phase2(variant: str) -> Dict[str, int]:
    """
    ...
    for motif in ['in_feedback_loop', 'in_feedforward_loop', 'in_single_input_module', 'in_cascade']:
        ...
        if len(df_motif) > 0:
            ...
            feature_name = df_motif.loc[best_idx, 'latent_idx']
            ...
            print(f"  {motif}: Feature z{feature_idx} (rpb={df_motif.loc[best_idx, 'rpb']:.3f})")
        else:
            print(f"  {motif}: No significant features found, using z0")
            top_features[motif] = 0

    return top_features


def load_test_graphs_with_features(max_graphs: int = 200, min_motif_ratio: float = 0.2, max_motif_ratio: float = 0.8) -> List[Data]:
    """
...
def run_variant_comparison(variant: str, device: str = 'cuda') -> pd.DataFrame:
    ...
    top_features = load_top_features_from_phase2(variant)

    # Load test graphs
    print("\n5. Loading test graphs...")
    test_graphs = load_test_graphs_with_features(max_graphs=200, min_motif_ratio=0.2, max_motif_ratio=0.8)
