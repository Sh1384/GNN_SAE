...
def build_checkpoint_path(variant, config):
    ...
    if variant == 'topk':
        ...
        total_latent = int(config['num_experts']) * int(config['latent_per_expert'])
        path = f"checkpoints/sae_switch_experts{int(config['num_experts'])}_latent{total_latent}_k{int(config['k_per_expert'])}_seed42.pt"
    else:
...
def extract_latent_representations(model, test_graph_ids, latent_dim):
    ...
    for graph_id in tqdm(test_graph_ids, desc="Processing test graphs"):
        ...
        # Move activations to the same device as the model
...
def plot_correlation_distributions(df_corr, output_dir):
    ...
    ax = axes[1]
    if len(df_pr) > 0 and 'motif' in df_pr.columns:
        for motif in motif_types:
        ...
        # If no precision-recall data, just show a message
        ax.text(0.5, 0.5, 'Precision-Recall data not available', 
...
def main():
    ...
    # Construct config name from available fields (config_name may not exist)
    if 'config_name' in best_config and pd.notna(best_config['config_name']):
        print(f"  Config: {best_config['config_name']}")
    else:
        # Build config description from available parameters
        config_parts = [f"latent_dim={int(best_config['latent_dim'])}"]
        if args.variant == 'topk' and 'k' in best_config and pd.notna(best_config['k']):
            config_parts.append(f"k={int(best_config['k'])}")
        elif args.variant == 'gated' and 'sparsity_coef' in best_config and pd.notna(best_config['sparsity_coef']):
            config_parts.append(f"lambda={best_config['sparsity_coef']:.0e}")
        elif args.variant == 'jumprelu':
            if 'threshold_init' in best_config and pd.notna(best_config['threshold_init']):
                config_parts.append(f"thresh={best_config['threshold_init']:.0e}")
            if 'bandwidth' in best_config and pd.notna(best_config['bandwidth']):
                config_parts.append(f"bw={best_config['bandwidth']:.0e}")
        elif args.variant == 'switch':
            if 'num_experts' in best_config and pd.notna(best_config['num_experts']):
                config_parts.append(f"experts={int(best_config['num_experts'])}")
            if 'latent_per_expert' in best_config and pd.notna(best_config['latent_per_expert']):
                config_parts.append(f"latent_pe={int(best_config['latent_per_expert'])}")
            if 'k_per_expert' in best_config and pd.notna(best_config['k_per_expert']):
                config_parts.append(f"k_pe={int(best_config['k_per_expert'])}")
