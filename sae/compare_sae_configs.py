...
Examples:
  ...
  python compare_sae_configs.py --metric composite_score
    ...
    # Handle different sparsity parameter types
    import pandas as pd
    if pd.notna(best['k']):
        print(f"  k = {int(best['k'])}")
    elif pd.notna(best.get('k_per_expert')):
        print(f"  k_per_expert = {int(best['k_per_expert'])} (num_experts = {int(best['num_experts'])})")
    elif pd.notna(best.get('sparsity_coef')):
  ...
  To use the recommended configuration, check the full details above.
    """)
