...
1. Original: GNN inference using original layer3 activations (no SAE).
...
try:
    ...
    print(f"Warning: Could not import GCNModel from gnn_train_copy: {str(e)}")
    GCNModel = None
...
def load_sae_model(variant=None, checkpoint_path=None, latent_dim=None, **kwargs):
    ...
    if checkpoint_path is None:
        if variant == 'topk':
            ...
            bandwidth = kwargs.get('bandwidth', 0.01)
            checkpoint_path = f"checkpoints/sae_jumprelu_latent{latent_dim}_thresh{threshold_init:.0e}_bw{bandwidth:.0e}_seed42.pt"
        elif variant == 'switch':
    ...
    try:
        if variant == 'topk':
            model = TopKSAE(input_dim=80, latent_dim=latent_dim, k=kwargs.get('k'))
        elif variant == 'gated':
            model = GatedSAE(input_dim=80, latent_dim=latent_dim, sparsity_coef=kwargs.get('sparsity_coef', 1e-3))
        elif variant == 'jumprelu':
            model = JumpReLUSAE(input_dim=80, latent_dim=latent_dim,
                               threshold_init=kwargs.get('threshold_init', 0.01),
                               bandwidth=kwargs.get('bandwidth', 0.01))
        elif variant == 'switch':
            model = SwitchSAE(input_dim=80, num_experts=kwargs.get('num_experts', 8),
                             latent_per_expert=kwargs.get('latent_per_expert', 64),
                             k_per_expert=kwargs.get('k_per_expert', 8))
        else:
...
def load_gnn_model():
    ...
    try:
        ...
        print(f"Error: Could not import GCNModel from gnn_train_copy: {str(e)}")
        return None
    ...
    try:
        ...
        gnn = GCNModel(input_dim=2, hidden_dim=hidden_dim, output_dim=1, dropout=0.5)
...
def evaluate_gnn_output(gnn_model, layer3_activations, edge_index, edge_weight):
    """Evaluate GNN output for given layer3 activations.

    Args:
        gnn_model: GNN model (or None)
        layer3_activations: Tensor of layer3 activations (output of conv3, 64-dim)
        edge_index: Edge indices
        edge_weight: Edge weights

    Returns:
        Predictions tensor (output of conv4) or None if gnn_model is None or inference fails
    """
    ...
    try:
        with torch.no_grad():
            # Apply conv4 to layer3 activations to get final predictions
            pred = gnn_model.conv4(layer3_activations, edge_index, edge_weight=edge_weight)
            pred = pred.squeeze(-1)

        # Validate output
        if pred.shape[0] != layer3_activations.shape[0]:
            print(f"Error: GNN output shape mismatch: expected {layer3_activations.shape[0]} predictions, got {pred.shape[0]}")
            return None
...
def run_ablation_experiment(variant, latent_dim, ablate_indices, experiment_name, motif_type_filter=None, use_mixed_motifs=False, variant_kwargs=None):
    ...
    Args:
        ...
        variant_kwargs: Dict of variant-specific parameters (k, sparsity_coef, threshold_init, num_experts, etc.)
    ...
    # Load SAE model
    try:
        sae_model = load_sae_model(variant=variant, latent_dim=latent_dim, **variant_kwargs)
    except Exception as e:
    ...
    try:
        if use_mixed_motifs:
            ...
            # These should be in outputs/activations/layer2_new/mixed/
...
def main():
    ...
    Command-line interface:
        python run_ablation.py --variant topk --latent_dim 512 --k 16 --feature z496
        python run_ablation.py --variant gated --latent_dim 512 --feature z496
    """
    ...
    parser.add_argument('--variant', type=str, choices=['topk', 'gated', 'jumprelu', 'switch'],
                       default='topk', help='SAE variant (default: topk)')
    parser.add_argument('--latent_dim', type=int, required=True, help='SAE latent dimension')
    parser.add_argument('--k', type=int, default=None, help='TopK parameter (required for topk variant)')
    parser.add_argument('--sparsity_coef', type=float, default=None, help='Sparsity coefficient (for gated variant)')
    parser.add_argument('--threshold_init', type=float, default=None, help='Threshold initialization (for jumprelu variant)')
    parser.add_argument('--bandwidth', type=float, default=None, help='Bandwidth for straight-through estimator (for jumprelu variant)')
    parser.add_argument('--num_experts', type=int, default=None, help='Number of experts (for switch variant)')
    parser.add_argument('--latent_per_expert', type=int, default=None, help='Latent dim per expert (for switch variant)')
    parser.add_argument('--k_per_expert', type=int, default=None, help='K per expert (for switch variant)')
    ...
    if args.variant == 'topk' and args.k is None:
        print("ERROR: --k is required for topk variant")
        return 1
    if args.variant == 'gated' and args.sparsity_coef is None:
        print("ERROR: --sparsity_coef is required for gated variant")
        return 1
    if args.variant == 'jumprelu' and args.threshold_init is None:
        print("ERROR: --threshold_init is required for jumprelu variant")
        return 1
    if args.variant == 'jumprelu' and args.bandwidth is None:
        # Set default bandwidth if not provided
        args.bandwidth = 0.01
    if args.variant == 'switch' and (args.num_experts is None or args.latent_per_expert is None or args.k_per_expert is None):
        print("ERROR: --num_experts, --latent_per_expert, and --k_per_expert are required for switch variant")
        return 1
    ...
    try:
        # Build variant-specific kwargs
        variant_kwargs = {}
        if args.variant == 'topk':
            variant_kwargs['k'] = args.k
        elif args.variant == 'gated':
            variant_kwargs['sparsity_coef'] = args.sparsity_coef
        elif args.variant == 'jumprelu':
            variant_kwargs['threshold_init'] = args.threshold_init
            variant_kwargs['bandwidth'] = args.bandwidth
        elif args.variant == 'switch':
            variant_kwargs['num_experts'] = args.num_experts
            variant_kwargs['latent_per_expert'] = args.latent_per_expert
            variant_kwargs['k_per_expert'] = args.k_per_expert

        # Run Analysis
        df = run_ablation_experiment(
            args.variant, args.latent_dim, ablate_indices, experiment_name,
            args.motif_type, args.use_mixed_motifs, variant_kwargs
        )
    except Exception as e:
