...
#### **Variant-Specific Filename Conventions**

Phase 3a generates filenames that **include all variant-specific hyperparameters** to prevent collisions when running with different configurations. The filename structure varies by variant:

**TopK Variant**:
```
ablations/results/feedback_loop_topk_l512_k8_results.csv
                 ↑ motif     ↑ variant ↑L ↑K ← embedded parameters
```
- Parameters: `latent_dim` (L), `k` (K)
- Example: `feedback_loop_topk_l512_k8_results.csv`

**Gated Variant**:
```
ablations/results/cascade_gated_l256_lambda1e-03_results.csv
                 ↑ motif ↑ variant ↑L ↑sparsity_coef
```
- Parameters: `latent_dim` (L), `sparsity_coef` (lambda, scientific notation)
- Example: `cascade_gated_l256_lambda1e-03_results.csv`

**JumpReLU Variant**:
```
ablations/results/feedforward_loop_jumprelu_l512_thresh1e-02_bw1e-02_results.csv
                 ↑ motif          ↑ variant   ↑L ↑threshold_init   ↑bandwidth
```
- Parameters: `latent_dim` (L), `threshold_init` (thresh), `bandwidth` (bw), all in scientific notation
- Example: `feedforward_loop_jumprelu_l512_thresh1e-02_bw1e-02_results.csv`

**Switch Variant**:
```
ablations/results/single_input_module_switch_l512_experts8_latentpeexp64_k8_results.csv
                 ↑ motif             ↑ variant ↑L ↑num_experts ↑latent_per_expert ↑k_per_expert
```
- Parameters: `latent_dim` (L), `num_experts`, `latent_per_expert`, `k_per_expert`
- Note: Total latent_dim = num_experts × latent_per_expert
- Example: `single_input_module_switch_l512_experts8_latentpeexp64_k8_results.csv`

**Why Full Parameters in Filenames?**
- Prevents silent collisions: Different parameter values create different filenames
- Example: If running JumpReLU with threshold_init=0.01 vs 0.1, old code would overwrite files with same name
- Phase 3c can correctly reconstruct filenames without hardcoding assumptions about which parameters were used

---

#### **phase_3a_config.json Structure**

Phase 3a automatically saves metadata to enable Phase 3c/3d to reconstruct exact filenames:
...
```json
{
  ...
  "min_rpb": 0.05,
  "n_random_trials": 20,
  "use_mixed_motifs": false,
  "results_dir": "ablations/interpretability_topk_latent512_k8_rpb0.05_results",
  "plots_dir": "ablations/interpretability_topk_latent512_k8_rpb0.05_plots"
}
```

**For each variant, variant_kwargs contains**:
- **TopK**: `{"k": 8}`
- **Gated**: `{"sparsity_coef": 0.001}`
- **JumpReLU**: `{"threshold_init": 0.01, "bandwidth": 0.01}`
- **Switch**: `{"num_experts": 8, "latent_per_expert": 64, "k_per_expert": 8}`

**Purpose**: Phase 3c reads this file to reconstruct filenames using the EXACT same parameters Phase 3a used, eliminating filename mismatches.
...
**Input**: Phase 3a + Phase 3b outputs + `phase_3a_config.json`
**Output**: Agreement metrics + comparison plots
**Time**: ~20 min (Option A) or ~80 min (Option B)

**Script**: `compare_ablation_strategies.py`

**CRITICAL**: Phase 3c automatically loads `phase_3a_config.json` to reconstruct the exact filenames Phase 3a created. This ensures filenames match even with variant-specific parameters.

---

#### **How Phase 3c Reconstructs Filenames**

When loading results from Phase 3a, Phase 3c:
1. Reads `phase_3a_config.json` to get: `variant`, `latent_dim`, `variant_kwargs`
2. Uses `generate_sae_motif_filename()` helper function to reconstruct expected filename
3. Looks for results in `ablations/results/{reconstructed_filename}`

**Example**: If phase_3a_config.json contains:
```json
{
  "variant": "jumprelu",
  "latent_dim": 512,
...
Phase 3c generates filename: `feedback_loop_jumprelu_l512_thresh1e-02_bw1e-02_results.csv`

This approach prevents the silent failure mode where Phase 3a creates files with parameters, but Phase 3c can't find them because it hardcodes different naming assumptions.
...
**What it does**:
- Loads best variant from Phase 3a metadata (`phase_3a_config.json`)
- Reconstructs correct filenames using variant parameters
- Compares SAE vs native ablation impacts for THAT VARIANT ONLY
- Measures per-motif agreement (Pearson/Spearman correlation)

**How to run**:
```bash
# Automatically loads variant from phase_3a_config.json
python compare_ablation_strategies.py --motif-mode
```

Or explicitly:
```bash
# Override with specific variant (still uses phase_3a_config.json for parameters)
