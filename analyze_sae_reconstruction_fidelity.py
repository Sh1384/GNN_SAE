...
def load_sae_model(variant: str, config: Dict, device: str = 'cuda') -> nn.Module:
    ...
    if variant == 'topk':
        ...
        ckpt_name = f"sae_switch_experts{config['num_experts']}_latent{config['latent_per_expert']}_k{config['k_per_expert']}_seed42.pt"
    ...
    if variant == 'topk':
        model = TopKSAE(input_dim=64, latent_dim=config['latent_dim'], k=config['k'])
    elif variant == 'gated':
        model = GatedSAE(input_dim=64, latent_dim=config['latent_dim'], sparsity_coef=config['sparsity_coef'])
    elif variant == 'jumprelu':
        model = JumpReLUSAE(input_dim=64, latent_dim=config['latent_dim'],
                           threshold_init=config['threshold_init'],
                           bandwidth=config['bandwidth'])
    elif variant == 'switch':
        model = SwitchSAE(input_dim=64,
...
def load_activations(graph_ids: List[int], use_mixed_motifs: bool = False) -> torch.Tensor:
    ...
    if use_mixed_motifs:
        activation_dir = Path("outputs/activations/layer2/mixed")
    else:
