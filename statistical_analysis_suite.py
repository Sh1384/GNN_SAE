...
import torch
from scipy.stats import pointbiserialr, wilcoxon, ranksums
    ...
    def analyze_stability(self, variant: str, config: Dict, seeds: List[int]) -> Dict:
        ...
        for seed in seeds:
            ...
            if variant == 'topk':
                ...
                ckpt_path = f"checkpoints/sae_jumprelu_latent{config['latent_dim']}_thresh{config['threshold_init']:.0e}_bw1e-02_seed{seed}.pt"
            elif variant == 'switch':
...
class AblationConditionalAnalyzer:
    """Analyzes ablation effects conditioned on motif presence/absence."""

    def analyze_conditional_ablation(self, feature_idx: int, variant: str, motif: str) -> Dict:
        """
        Analyze ablation effect separately for graphs with and without target motif.

        Returns:
            Dict with Wilcoxon test results and effect sizes
        """
        # Load actual ablation results from run_ablation.py outputs
        ablation_file = Path("ablations") / "results" / f"ablation_{variant}_feature{feature_idx}.csv"

        if not ablation_file.exists():
            # If ablation file not found, return placeholder with warning
            print(f"  Warning: Ablation file not found: {ablation_file}")
            return {
                'feature_idx': feature_idx,
                'variant': variant,
                'motif': motif,
                'error': f'Ablation file not found',
            }
        ...
        try:
            ...
            # Load motif metadata to identify graphs with/without target motif
            metadata_dir = Path("virtual_graphs/data/all_graphs/graph_motif_metadata")

            with_motif_delta_mse = []
            without_motif_delta_mse = []

            for _, row in df_ablation.iterrows():
                graph_id = row['graph_id']
                delta_mse = row.get('delta_mse', row.get('delta_loss', 0))
                ...
                if metadata_file.exists():
                    ...
                    has_motif = (df_meta[motif.replace('in_', '')] if motif.replace('in_', '') in df_meta.columns
                                else df_meta.get(motif, 0)).any()

                    if has_motif:
                        with_motif_delta_mse.append(delta_mse)
                    else:
                        without_motif_delta_mse.append(delta_mse)

            # Handle case where we don't have enough samples
            with_motif_delta_mse = np.array(with_motif_delta_mse) if with_motif_delta_mse else np.array([0.0])
            without_motif_delta_mse = np.array(without_motif_delta_mse) if without_motif_delta_mse else np.array([0.0])

        except Exception as e:
            print(f"  Error loading ablation data for {variant} feature {feature_idx}: {e}")
            return {
                'feature_idx': feature_idx,
                'variant': variant,
                'motif': motif,
        ...
        try:
            stat, p_value = wilcoxon(with_motif_delta_mse, without_motif_delta_mse)
        except ValueError:
            # wilcoxon() requires paired samples of same length; if it fails, use unpaired test
            stat, p_value = ranksums(with_motif_delta_mse, without_motif_delta_mse)

        # Effect size (Cohen's d)
        cohens_d = (with_motif_delta_mse.mean() - without_motif_delta_mse.mean()) / \
                  np.sqrt((with_motif_delta_mse.std()**2 + without_motif_delta_mse.std()**2) / 2)

        return {
            'feature_idx': feature_idx,
            'variant': variant,
            'motif': motif,
            'delta_mse_with_motif_mean': with_motif_delta_mse.mean(),
            'delta_mse_without_motif_mean': without_motif_delta_mse.mean(),
            'delta_mse_with_motif_std': with_motif_delta_mse.std(),
            'delta_mse_without_motif_std': without_motif_delta_mse.std(),
            'wilcoxon_stat': stat,
            'wilcoxon_p': p_value,
            'cohens_d': cohens_d,
    ...
    def plot_conditional_effects(self, results: List[Dict]):
        """Plot ablation effects conditioned on motif presence."""
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle('Ablation Effects Conditioned on Motif Presence',
                    fontsize=16, fontweight='bold')

        motifs = ['in_feedforward_loop', 'in_feedback_loop', 'in_single_input_module', 'in_cascade']
        motif_names = ['Feedforward Loop', 'Feedback Loop', 'Single-Input Module', 'Cascade']

        for idx, (motif, motif_name) in enumerate(zip(motifs, motif_names)):
            ax = axes[idx // 2, idx % 2]

            # Filter results for this motif
            motif_results = [r for r in results if r.get('motif') == motif]

            if len(motif_results) > 0:
                variants = list(set(r['variant'] for r in motif_results))
                x_pos = np.arange(len(variants))
                width = 0.35

                with_means = [next((r['delta_mse_with_motif_mean'] for r in motif_results if r['variant'] == v), 0)
                             for v in variants]
                without_means = [next((r['delta_mse_without_motif_mean'] for r in motif_results if r['variant'] == v), 0)
                                for v in variants]

                ax.bar(x_pos - width/2, with_means, width, label='With motif', alpha=0.7, edgecolor='black')
                ax.bar(x_pos + width/2, without_means, width, label='Without motif', alpha=0.7, edgecolor='black')

                ax.set_xlabel('Variant', fontsize=11)
                ax.set_ylabel('Δ MSE', fontsize=11)
                ax.set_title(motif_name, fontsize=12, fontweight='bold')
                ax.set_xticks(x_pos)
        ...
        plt.tight_layout()
        plt.savefig(STATS_DIR / 'ablation_conditional_effects.png', dpi=300, bbox_inches='tight')
        print("✓ Saved ablation_conditional_effects.png")
        plt.close()
...
class FeatureRedundancyAnalyzer:
    ...
    def analyze_redundancy(self, variant: str, config: Dict) -> Dict:
        ...
        if variant == 'topk':
            ...
            ckpt_path = f"checkpoints/sae_jumprelu_latent{config['latent_dim']}_thresh{config['threshold_init']:.0e}_bw1e-02_seed42.pt"
        elif variant == 'switch':
            total_latent = config['num_experts'] * config['latent_per_expert']
...
def main():
    ...
    # 3. Ablation Conditional Analysis
    if args.ablation_type in ['latent', 'both']:
        print("\n3. Analyzing ablation effects conditioned on motif presence...")
        ablation_analyzer = AblationConditionalAnalyzer()
        results = []
        motifs = ['in_feedforward_loop', 'in_feedback_loop', 'in_single_input_module', 'in_cascade']
        for motif in motifs:
            for variant in ['topk', 'gated', 'jumprelu', 'switch']:
                results.append(ablation_analyzer.analyze_conditional_ablation(0, variant, motif))
        ablation_analyzer.plot_conditional_effects(results)
